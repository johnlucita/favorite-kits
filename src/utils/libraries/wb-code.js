...
class WbCode {
     ...
     */
    GetWbFirstCodeResult (arg) {
        let result = ''
        for (let i = 0; i < arg.length; i++) {
            const code = arg.charAt(i)
            const _unicode = code.charCodeAt(0)
            if (_unicode >= 1 && _unicode <= 254) {
                result += code
            } else {
                for (let j = 0; j < global_dictionary.length; j++) {
                    const chars = global_dictionary[j].split('')
                    for (let k = 1; k < chars.length; k++) {
                        if (_unicode === chars[k].charCodeAt(0)) {
                            if (j === global_dictionary.length - 1) {
                                result += chars[k]
                            } else {
                                result += chars[0]
                            }
...
export default arg => {
    ...
    const _wbCode = new WbCode()
    return _wbCode.GetWbFirstCodeResult(_value)
}
